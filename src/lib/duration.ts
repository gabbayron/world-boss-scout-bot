 ...
 */
export function parseLayerOpenDuration(input: string): number | null {
  ...
  if (!/^(\d+[dhm])+$/i.test(compact)) return null;
  ...
  while ((m = re.exec(compact)) !== null) {
    ...
    if (u === "d") total += n * MS_DAY;
    else if (u === "h") total += n * MS_HOUR;
    else if (u === "m") total += n * MS_MIN;
  }
